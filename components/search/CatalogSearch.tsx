    ...
    <>
      <div className="flex gap-2 items-center flex-wrap">
        <div className="flex gap-2 items-center w-full flex-nowrap">
          ...
          <div className="grid grid-cols-3 max-w-[300px] w-full gap-2">
            <Filter
              name="Subject"
              index={1}
      ...
      {({ open }) => (
        <>
          <PopoverButton className={`
            ...
            group w-full justify-center flex items-center rounded-md bg-zinc-400/50 dark:bg-zinc-700 px-2 py-2 text-sm font-medium hover:text-zinc-900 dark:hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 whitespace-nowrap`}>
            <span>{buttonDisplay}</span>
            <HiChevronDown
              className={`${open ? 'text-zinc-600 dark:text-zinc-300' : 'text-zinc-600/50 dark:text-zinc-300/50'}
                ml-2 size-4 transition duration-150 ease-in-out dark:group-hover:text-zinc-300/80 stroke-2`}
              aria-hidden="true"
            />
          </PopoverButton>
          <Transition
...
function FilterToggle({ handler, content, allToggles, updateToggle }: {handler: (e: React.ChangeEvent<HTMLInputElement>) => void, content: string, allToggles: {
  text: string,
  enabled: boolean
  ...
  return (
    <div className="flex flex-nowrap p-1">
       <label className="text-base whitespace-nowrap" >
          <input type="checkbox" name={content} className="w-6 h-6 text-xs py-1 px-2 rounded transition mr-3" checked={isChecked} onChange={(e) => { updateCheckbox(e)}} />
          {content}
        </label>
    </div>
